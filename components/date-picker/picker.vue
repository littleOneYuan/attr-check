      ...
      <Drop @click.native="handleTransferClick"
        ...
        <div>
          <component :is="panel"
                     ...
                     @on-selection-mode-change="onSelectionModeChange"></component>
        </div>
      </Drop>
    </transition>
  </div>
...
<script>
import Drop from './base/dropdown.vue'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      isFocused: false,
      focusedDate: initialValue[1] || this.startDate || new Date(),
      focusedTime: {
  ...
  methods: {
    ...
    navigateDatePanel (direction, shift) {
      ...
      const initialDate = this.focusedDate || (this.internalValue && this.internalValue[1]) || new Date()
    ...
    onPick (dates, visible = false, type) {
      ...
      if (this.internalValue[1]) this.focusedDate = this.internalValue[1]
      this.focusedTime = {...this.focusedTime, time: this.internalValue.map(extractTime)}
