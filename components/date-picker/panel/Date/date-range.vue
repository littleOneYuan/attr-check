  ...
  <div :class="classes"
    ...
    <div :class="[prefixCls + '-sidebar']"
      ...
      <div :class="[prefixCls + '-shortcut']"
           v-for="(shortcut,id) in shortcuts"
           :key="id"
...
export default {
  ...
  data () {
    ...
    const leftPanelDate = this.startDate ? this.startDate : maxDate
    ...
    return {
      ...
      leftPanelDate: new Date(leftPanelDate.getFullYear(), leftPanelDate.getMonth() - 1, 1)
    }
  },
  computed: {
    classes () {
  ...
  watch: {
    value (newVal) {
      ...
      this.setPanelDates(this.startDate || this.dates[1] || new Date())
    },
  ...
  methods: {
    ...
    setPanelDates (rightPanelDate) {
      this.rightPanelDate = rightPanelDate
      const leftPanelDate = new Date(rightPanelDate.getFullYear(), rightPanelDate.getMonth() - 1, 1)
      const splitRightPanelDate = this.dates[0] ? this.dates[0].getTime() : this.dates[0]
      this.leftPanelDate = this.splitPanels ? new Date(Math.min(splitRightPanelDate, leftPanelDate.getTime())) : leftPanelDate
    },
    ...
    handleRangePick (val, type) {
      if (this.rangeState.selecting || this.currentView === 'time') {
        ...
        // send shortcut clear event
        this.$emit('on-clear-shortcut')
        this.rangeState = {
          from: val,
          to: null,
          selecting: true
